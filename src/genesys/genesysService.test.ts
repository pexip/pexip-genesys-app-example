...
describe('Genesys service', () => {
  ...
  describe('initialize', () => {
    it('should set the environment', async () => {
      ...
      expect(PlatformClient.ApiClient.instance.setEnvironment).toBeCalledTimes(
        1
      )
      expect(PlatformClient.ApiClient.instance.setEnvironment).toBeCalledWith(
        state.pcEnvironment
      )
    })
    ...
    it('should set the access token', async () => {
      await GenesysService.initialize(state, accessToken)
      expect(PlatformClient.ApiClient.instance.setAccessToken).toBeCalledTimes(
        1
      )
      expect(PlatformClient.ApiClient.instance.setAccessToken).toBeCalledWith(
        accessToken
      )
    })
  })
